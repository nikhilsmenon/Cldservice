...
docker run -d -it --restart=always  --env-file /var/ecr.env  -p 5000:5000  --name cwcget $cwc_image
...
#sleep 10
#docker wait cwcget
CID=$(docker ps -q -f status=running -f name=^/cwcget)
if [ ! "${CID}" ]; then
